...
heights = np.zeros(shape=(num_rows, num_cols))
visible = np.zeros(shape=(num_rows, num_cols))
for y in range(0, num_rows):
    for x in range(0, num_cols):
        ...
        west_max = max_of_array(west)
        east_max = max_of_array(east)
        north_max = max_of_array(north)
        south_max = max_of_array(south)

        min_max = min(west_max, east_max, north_max, south_max)

        if height > min_max or x == 0 or x == num_cols - 1 or y == 0 or y == num_rows - 1:
            v = 1
        else:
            v = 0
        visible[y, x] = int(v)
#print(visible)
visible_trees = int(visible.sum())
print(visible_trees)
...
"""
def trees_visible(height, array):
    if len(array) == 0:
        return 0
...
scores = np.zeros(shape=(num_rows, num_cols))
...
for y in range(0, num_rows):
    for x in range(0, num_cols):
        ...
        west_trees = trees_visible(height, west)
        east_trees = trees_visible(height, east)
        north_trees = trees_visible(height, north)
        south_trees = trees_visible(height, south)

        score = west_trees * east_trees * north_trees * south_trees
        scores[y, x] = score
