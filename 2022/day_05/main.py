...
for line in raw:
    if line[0] == 'm':          # record instructions
        a = line.split('move ')[1].split(' ')[0]
        b = line.split('from ')[1].split(' ')[0]
        c = line.split('to ')[1].split('\n')[0]
        instructions.append([int(a), int(b), int(c)])
    else:                       # record initial stack
        lst = [x for x in line]
        for i in range(1, len(stack)+1):
            j = i*4 - 3
            if len(lst) > j:
                if lst[j].isalpha():
                    stack[i] = [lst[j]] + stack[i]
print(stack)
print(instructions)


def run_instructions(instructions, stack, multiple_boxes):

    for inst in instructions:
        cnt = inst[0]
        src = inst[1]
        des = inst[2]
        src_len = len(stack[src])
        if multiple_boxes:
            stack[des] = stack[des] + stack[src][src_len - cnt:]
        else:
            stack[des] = stack[des] + stack[src][src_len-cnt:][::-1]
        stack[src] = stack[src][:src_len-cnt]
    return stack


def get_top_crates(stack):
    top_crates = ''
    for k in stack:
        if len(stack[k]) == 0:
            continue
        else:
            top_crates = top_crates + stack[k][-1]
    return top_crates


stack_1 = stack.copy()
new_stack_1 = run_instructions(instructions, stack_1, multiple_boxes=False)
top_crates_1 = get_top_crates(new_stack_1)
print(top_crates_1)
# TDCHVHJTG
...
"""
stack_2 = stack.copy()
new_stack_2 = run_instructions(instructions, stack_2, multiple_boxes=True)
top_crates_2 = get_top_crates(new_stack_2)
print(top_crates_2)
