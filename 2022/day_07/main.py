    ...
    for line in file:
        if '$ ls' in line:
            ...
            if '$ cd /' in line:
                ...
                path = path.rsplit('/', 1)[0].rsplit('/', 1)[0] + '/'
            else:
                path = path + line.replace('$ cd ', '').replace('\n', '') + '/'
        elif 'dir' in line:
            full_directory.append([path, 0])
        else:
            filesize = int(line.split(' ')[0])
            full_directory.append([path, filesize])

#print(full_directory)

totals = {}
for i in full_directory:
    directory = i[0]
    total = 0
    if directory in totals:
        continue
    else:
        for j in full_directory:
            if j[0].startswith(directory):
                total += j[1]
...
maximum = 100000
filtered_total = {k: v for k, v in totals.items() if v <= maximum}
#print(filtered_total)
print(sum(filtered_total.values()))
...
delete_required = space_required - free_space
directories_big_enough = {k: v for k, v in totals.items() if v >= delete_required}
print(min(directories_big_enough.values()))