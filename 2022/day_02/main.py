...
code_map = pd.DataFrame({'encoded_strategy': ['X', 'Y', 'Z'], 'chosen_weapon': ['A', 'B', 'C']})

base_score = pd.DataFrame({'chosen_weapon': ['A', 'B', 'C'], 'base_score': [1, 2, 3]})
result_score = pd.DataFrame({'result': ['win', 'draw', 'loss'], 'result_score': [6, 3, 0]})
result_map = pd.DataFrame({'target_weapon': ['A', 'A', 'A', 'B', 'B', 'B', 'C', 'C', 'C'],
                           'chosen_weapon': ['A', 'B', 'C', 'A', 'B', 'C', 'A', 'B', 'C'],
                           'result': ['draw', 'win', 'loss', 'loss', 'draw', 'win', 'win', 'loss', 'draw']})
...
data = pd.DataFrame({'target_weapon': x, 'encoded_strategy': y})

df = pd.merge(data, code_map, how='inner', on='encoded_strategy')
df = pd.merge(df, result_map, how='inner', on=['target_weapon', 'chosen_weapon'])
df = pd.merge(df, base_score, how='inner', on='chosen_weapon')
df = pd.merge(df, result_score, how='inner', on='result')
df['game_score'] = df['base_score'] + df['result_score']
#print(df)

total_score = df['game_score'].sum()
print(total_score)
...
new_code_map = pd.DataFrame({'encoded_strategy': ['X', 'Y', 'Z'], 'result': ['loss', 'draw', 'win']})


df = pd.merge(data, new_code_map, how='inner', on='encoded_strategy')
df = pd.merge(df, result_map, how='inner', on=['result', 'target_weapon'])
df = pd.merge(df, base_score, how='inner', on='chosen_weapon')
df = pd.merge(df, result_score, how='inner', on='result')
df['game_score'] = df['base_score'] + df['result_score']
#print(df)
